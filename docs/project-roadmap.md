...
| Hero visual | Claude | ✅ **DONE 2026-06-24** — Charlie's concept: the browser-frame mockup now **loops "Coming soon" → a Charged-style website building itself in** (staggered nav/headline/button/media/cards) → back. Pure coded animation, `components/HeroMockup.tsx` (framer-motion, reduced-motion shows the finished layout statically). No asset/video needed; removed the old placeholder + empty `<video>` slot. (Earlier parked image-gen/video idea dropped in favour of this.) |
