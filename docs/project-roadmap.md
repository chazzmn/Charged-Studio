...
- **PERF regression fix** — Lighthouse "Legacy JavaScript" (~14 KiB polyfills: Array.at, Object.hasOwn, flat/flatMap, fromEntries, trimStart/End). Cause: `browserslist` was `["last 2 years","not dead"]` — the `"not dead"` half is an OR that re-admits old browsers, so Next re-emitted the polyfill chunk. Fixed to explicit modern targets (chrome/edge ≥93, firefox ≥92, safari/ios ≥15.4) — verified via `npx browserslist` to resolve modern-only. **Needs redeploy + re-test** (can't `next build` in sandbox). Don't re-add `"not dead"`/`"last N years"` broad queries.
- **WebMCP (AEO)** — `components/WebMCP.tsx` (mounted in `layout.tsx`) registers the lead forms as agent tools via `navigator.modelContext.provideContext` (W3C WebMCP). Two tools w/ valid JSON inputSchema: `contact-charged-studio` + `request-website-audit`, both POST `/api/contact`. Feature-detected → no-op where unsupported. Satisfies the AEO checklist (form coverage / tools registered / schemas valid). Spec: webmachinelearning.github.io/webmcp.
