 ...
 * the root is a `.reveal-stagger` container and gets `.in` when `show` is true
 * (driven by the hero scene's active state, or simply true in the services rows).
 */
...
export function WebsiteGraphic({ show = true }: { show?: boolean }) {
  return (
    <div className={root(show, "flex h-full w-full flex-col gap-3 p-4 sm:gap-3.5 sm:p-5")}>
      ...
      <div className="grid flex-1 grid-cols-5 gap-3">
        <div className="col-span-3 flex flex-col justify-center gap-2.5">
          <span className="h-3 w-[88%] rounded bg-text/25" />
          <span className="h-3 w-[62%] rounded bg-text/25" />
          <span className="mt-1 h-1.5 w-[72%] rounded-full bg-text/10" />
          <span className="mt-2 h-5 w-20 rounded-md bg-accent" />
        </div>
        <div className="col-span-2 rounded-lg bg-surface-raised ring-1 ring-border" />
      </div>
      <div className="grid grid-cols-3 gap-3">
        <span className="h-8 rounded-md bg-surface-raised ring-1 ring-border sm:h-9" />
        <span className="h-8 rounded-md bg-surface-raised ring-1 ring-border sm:h-9" />
        <span className="h-8 rounded-md bg-surface-raised ring-1 ring-border sm:h-9" />
      </div>
    </div>
...
export function SearchGraphic({ show = true }: { show?: boolean }) {
  return (
    <div className={root(show, "flex h-full w-full flex-col gap-3 p-4 sm:p-5")}>
...
export function SoftwareGraphic({ show = true }: { show?: boolean }) {
  return (
    <div className={root(show, "flex h-full w-full gap-3 p-4 sm:p-5")}>
      <div className="flex w-10 flex-col gap-2 rounded-md bg-surface-raised p-2 ring-1 ring-border sm:w-12">
      ...
      <div className="flex flex-1 flex-col gap-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-2 rounded-md bg-surface-raised p-2.5 ring-1 ring-border">
            <span className="h-2 w-1/2 rounded-full bg-text/20" />
            <span className="h-4 w-3/4 rounded bg-text/30" />
          </div>
          <div className="flex items-end gap-1 rounded-md bg-surface-raised p-2.5 ring-1 ring-border">
        ...
        <div className="flex items-center justify-between rounded-md bg-surface-raised p-2.5 ring-1 ring-border">
        ...
        <div className="rounded-md bg-surface-raised p-2.5 ring-1 ring-border">
...
export function BrandingGraphic({ show = true }: { show?: boolean }) {
  return (
    <div className={root(show, "flex h-full w-full flex-col justify-center gap-4 p-5 sm:gap-5 sm:p-6")}>
